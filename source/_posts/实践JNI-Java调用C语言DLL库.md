...
下面的命令要在`Main.class`,`A.class`和`A.java`同时存在的目录下进行(生成头文件的命令是对.java文件进行的,而且同时需要访问到.class文件):

```bash
javah A
```

特别地,如果你的类文件是包含在一个`package`里的,你需要用`-classpath`参数指定整个项目编译出的class文件目录.

如果你使用jdk8以上,应该使用

```bash
javac -h ./ A # ./表示在当前目录生成头文件
```
